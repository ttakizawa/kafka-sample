...
# メイン : ターミナル出力用
def tm_main(interval, proc):
    print('ターミナル 出力')
    count = 0
    before = time.time()
    while True:
        current = time.time()
        if current - before >= interval:
            iotjsondata = iot_json_data(proc)
            json_dict = json.loads(iotjsondata)
            pprint.pprint(json_dict)
            before = current
            count += 1
            print(f"データ作成件数:{count}")



# メイン : Kafka出力用
def kf_main(interval, proc):
    print('Kafka 出力')
    count = 0
    before = time.time()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--interval', type=int, default=10, help='データ作成間隔(秒)')
    ...
    if (args.mode == 'kf'): 
        kf_main(args.interval, args.proc)
    else :
        tm_main(args.interval, args.proc)
